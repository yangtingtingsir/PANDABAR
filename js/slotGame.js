...
class SlotGame extends Phaser.Scene{
    ...
    preload(){
        ...
        slotConfig.symbols.forEach((s)=>{
            if(s.fileNameGUI != null) {
                // if(this.textures.exists(s.name + 'GUI')) this.textures.remove(s.name + 'GUI');        // uncomment for game with multiple slot scenes
                this.load.image(s.name + 'GUI', "png/Symbols/" + s.fileNameGUI);
            }
        });
    ...
    create(){   
        ...
        // 9a animate ballons
        this.animBalloonsLeftComplete = true;
        this.animBalloonsRightComplete = true;
        this.nextAnimTime = 0;

        this.setScreensClosed();
        this.showInfoScreen(true, 200, 500);

        // 10) debug FPS, pointer position
        // this.fpsText = this.add.bitmapText(this.centerX, this.centerY - 580, 'gameFont_2', 'fps: ', 40, 1).setOrigin(0.5);
        ...
        */
        ...
        // this.showWinCoinsMessage(20, 2000); // (coins, timeout)
        ...
        // var bwPU = this.guiController.showPopUp(slotConfig.createBigWinPUHandler); bwPU.messageText.text = 748;          // not used
        // var hwPU = this.guiController.showPopUp(slotConfig.createHugeWinPUHandler); hwPU.messageText.text = 3486;        // not used
        // var mwPU = this.guiController.showPopUp(slotConfig.createMegaWinPUHandler); mwPU.messageText.text = 1568;        // not used
        // var jpwPU = this.guiController.showPopUp(slotConfig.createJackpotWinPUHandler); jpwPU.messageText.text = 13690;  // not used
        // this.showCoins(true);
        // this.lampsBlink(true);
    }
    ...
    {   
        ...
        if(this.nextAnimTime < time)
        {
            // try to anim
            if(this.animBalloonsLeftComplete) this.animBalloonsLeft();
            if(this.animBalloonsRightComplete)this.animBalloonsRight();
            this.nextAnimTime += 2000;
        }
    ...
    runSlot()
    {
        ...
        if(this.playFreeSpins || this.slotControls.hasFreeSpin()) this.showScreen("freespinscreen", 0);
        else this.closeOpenedScreen();
        ...
        sA.add((callBack) =>{
            ...
            if(anyWin)
            {
                ...
                if(summCoins > 0) { this.showScreen("winscreen", 1500);}
                else if(winSpins > 0) { this.showScreen("freespinscreen", 1500);}                      
            }
            else
            {
                this.isCascadeSpin = false;
                if(!this.slotControls.auto && !this.slotControls.hasFreeSpin()) { this.showScreen("infoscreen", 0);}      
            }
    ...
    *winShowC(completeCallBack)
    {
        ...
        if (winSpins > 0) 
        {
            ...
            if(this.isFreeSpin) this.showWinFreeSpinsMessage(winSpins, slotConfig.winMessageTime);
            setTimeout(()=>
            { 
    ...
    *loseShowC(completeCallBack)
    ...
    animBalloonsLeft(){
        if(this.balloonsleft == null)return;
        if(Phaser.Math.Between(0, 10) < 3) return; // anim or no

        this.animBalloonsLeftComplete = false;
        this.angleLeftB = Phaser.Math.Between(-4, -8);
        this.durLeftB1 = Phaser.Math.Between(3000, 5000);
        this.durLeftB2 = Phaser.Math.Between(3000, 5000);
        // start 2 tweens
        this.tweens.add({
            targets: [this.balloonsleft],
            angle: this.angleLeftB,
            duration: this.durLeftB1,
            ease: "Sine.easeInOut",
            callbackScope: this,
            onComplete: function(tween)
            {
                this.tweens.add({
                    targets: [this.balloonsleft],
                    angle: this.balloonsleft.angle - this.angleLeftB,
                    duration: this.durLeftB2,
                    ease: "Sine.easeInOut",
                    callbackScope: this,
                    onComplete: function(tween)
                        {
    ...
    animBalloonsRight(){
        if(this.balloonsright == null) return;
        if(Phaser.Math.Between(0, 10) < 3) return; // anim or no

        this.animBalloonsRightComplete = false;
        this.angleRightB = Phaser.Math.Between(4, 8);
        this.durRightB1 = Phaser.Math.Between(3000, 5000);
        this.durRightB2 = Phaser.Math.Between(3000, 5000);
    
        // start 2 tweens
        this.tweens.add({
            targets: [this.balloonsright],
            angle: this.angleRightB,
            duration: this.durRightB1,
            ease: "Sine.easeInOut",
            callbackScope: this,
            onComplete: function(tween)
            {
                this.tweens.add({
                        targets: [this.balloonsright],
                        angle: this.balloonsright.angle - this.angleRightB,
                        duration: this.durRightB2,
                        ease: "Sine.easeInOut",
                        callbackScope: this,
                        onComplete: function(tween)
                            {
    ...
    // show screen: infoscreen, winscreen, freespinscreen
    showScreen(screen, timeDelay)
    {
        var oScreen = this.getOpenedScreen();
        if(oScreen === screen) return;
        var oDelay = timeDelay;
        if(oScreen !=='none') 
        {
            this.closeOpenedScreen();
            oDelay = (timeDelay > 300) ? timeDelay : 300;
        }
        if(screen.localeCompare("infoscreen") == 0) this.showInfoScreen(true, 200, oDelay);
        else if (screen.localeCompare("winscreen") == 0) this.showWinScreen(true, 200, oDelay);
        else if (screen.localeCompare("freespinscreen") == 0) this.showFreeSpinScreen(true, 200, oDelay);
    }

    showWinScreen(show, tweenTime, delayTime)
    {
        var screen = 'winscreen';
        if(this.screenTweenStarted) return;
        if(show && this[screen]) return;
        if(!show && !this[screen]) return;

        this[screen] = show;
        this.screenTweenStarted = true;
        var sT;
        this.delayAction(delayTime, ()=>{

            sT =  new SimpleTweenFloat(this, 1.0, 0.0, tweenTime,                                                        
                (v, dv) =>
                { 
                    this.slotControls.winPanel.setScale(1, show ? 1 - v : v);
                    this.slotControls.winAmountText.setScale(1, show ? 1 - v : v);
                },   
                ()=> {this.screenTweenStarted = false; });
        });
        
    }

    showFreeSpinScreen(show, tweenTime, delayTime)
    {
        console.log('show screen: freespinscreen' + show);
        var screen = 'freespinscreen';
        if(this.screenTweenStarted) return;
        if(show && this[screen]) return;
        if(!show && !this[screen]) return;
        ...
        this.delayAction(delayTime, ()=>{
            ...
            sT =  new SimpleTweenFloat(this, 1.0, 0.0, tweenTime,                                                        
                ...
                { 
                    this.slotControls.winPanel.setScale(1, show ? 1 - v : v);
                    this.slotControls.freeSpinCountText.setScale(1, show ? 1 - v : v); 
                },   
                ()=> {this.screenTweenStarted = false; });
        });
    ...
    showInfoScreen(show, tweenTime, delayTime)
    {
        ...
        this.delayAction(delayTime, ()=>{
            sT =  new SimpleTweenFloat(this, 1.0, 0.0, tweenTime,                                                        
                (v, dv) =>{ 
                    this.slotControls.winPanel.setScale(1, show ? 1 - v : v);
                    this.slotControls.infoText.setScale(1, show ? 1 - v : v);
                },   
                ()=> {this.screenTweenStarted = false; }); 
        });   
    }
    ...
    setScreensClosed()
    {
        this.slotControls.winPanel.setScale(1, 0);
        this.slotControls.winAmountText.setScale(1, 0);
        this.slotControls.infoText.setScale(1, 0);
        this.slotControls.freeSpinCountText.setScale(1, 0); 
        this['infoscreen'] = false;
        this['winscreen'] = false;
        this['freespinscreen']= false;
    }

    getOpenedScreen()
    {
       if(this['infoscreen']) return "infoscreen";
       else if (this['winscreen']) return "winscreen";
       else if (this['freespinscreen']) return "freespinscreen";
       else return 'none';
    }

    closeOpenedScreen()
    {
        if(this['infoscreen']) this.showInfoScreen(false, 200, 0)
        else if (this['winscreen']) this.showWinScreen(false, 200, 0)
        else if (this['freespinscreen']) this.showFreeSpinScreen(false, 200, 0)
    }

    delayAction(delayTime, callBack)
    {
        var sT =  new SimpleTweenFloat(this, 1.0, 0.0, delayTime,                                                        
            (v, dv) =>{ },   
...
function spinReels(reels, _slotConfig, completeCallback){
    ...
    reels.forEach((r)=>{
        pA.add((callBack)=>
        {
            var rand = (_slotConfig.reels_simulate && _slotConfig.reels_simulate[ri] >= 0) ? _slotConfig.reels_simulate[ri] : r.getRandomOrderPosition(); // && !slotConfig.Simulated - only once
            r.spin(rand, ()=>{callBack();}); 
            ri++;
        });         
    });
    pA.start(completeCallback);
    slotConfig.Simulated = true;
}
