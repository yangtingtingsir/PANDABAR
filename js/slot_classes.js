...
class Lamp
{
    constructor (scene, offsetX, offsetY, lampOffTexture, lampOnTexture)
    {
        this.lampOffTexture = lampOffTexture;
        this.lampOnTexture = lampOnTexture;
        this.scene = scene;
        this.lamp = scene.addSpriteLocPos(lampOffTexture, offsetX, offsetY);      
    }
...
class SlotSymbol
{
    ...
    scaleDeform(lY,dX)
    {
        var effect = 1;
        // scale deform 
        var x = this.sprite.x;         //  x coordinate
        var y =  this.sprite.y;         // y coordinate
        var dy = lY - y;
        var absDY = this.abs(dy);
        var relAbsDY = (absDY * effect) / 174;
        var scaleY = this.lerp(1.0, 0.7, relAbsDY); 
        var scaleX = this.lerp(1.0, 0.85, relAbsDY);
        this.sprite.setScale(scaleX, scaleY);
        var dPosX = this.lerp(dX, 0, relAbsDY);
        this.sprite.setPosition(this.posX + dPosX, Math.round(this.posY));
    }
...
class Reel{
    ...
    constructor (scene, reelData, reelNumber, symbolSizeY, windowsCount, randomStartPosition, spinTime, animFrameRate, maskSizeX, maskSizeY)
    {
        this.scene = scene;
        ...
        this.spinDeformX = slotConfig.reels[reelNumber].spinDeformX;
        this._create();
    }
    ...
    _create() 
    {
        ...
        this.visibleMaxY = (this.maskSizeY) ? this.posY + this.maskSizeY * 0.5 + this.symbolSizeY * 0.3 :  this.getWindowPosition(0).y + this.symbolSizeY * 0.8; // 0.6;
        this.visibleMinY = (this.maskSizeY) ? this.posY - this.maskSizeY * 0.5 - this.symbolSizeY * 0.3 : this.getWindowPosition(this.windowsCount - 1).y - this.symbolSizeY * 0.8; // 0.6;
       ...
       // this.scene['reel' + this.reelNumber].depth = -3;
        ...
        var mSizeX = (this.maskSizeX) ? this.maskSizeX : this.symbolSizeY * 1.2;
        var mSizeY = (this.maskSizeY) ? this.maskSizeY : this.windowsCount * this.symbolSizeY;

        shape.fillRect(this.posX - mSizeX * 0.5, this.windowPosY + mSizeY * 0.5,  mSizeX, -mSizeY);
        ...
        this.symbols.forEach((s)=>{s.setVisible(s.posY <= this.visibleMaxY && s.posY >= this.visibleMinY);   
            if(this.reelNumber != 10 && s.isVisible())  s.scaleDeform(this.posY, this.spinDeformX); 
        });
...
class SlotPlayer{
    ...
    setCoinsCount(count)
    {
        ...
        if (changed) 
        {
           ...
           // localStorage.setItem('mk_shopping_bl_amount', this.coins); // save coins
        }
    }
    ...
    loadCoins()
    {
       ...
       // amount = parseInt(localStorage.getItem('mk_shopping_bl_amount')) || this.defaultCoins; // load https://www.dynetisgames.com/2018/10/28/how-save-load-player-progress-localstorage/
...
class SlotControls
{
    constructor(scene, slotPlayer, linesData, lineColor, lineBetMaxValue, jackpotDefaultAmount)
    {
        ...
        this.holdToAutoSpin = false;                    // not allowed in current version, hold spin button pressed 2 sec to start auto spin mode
    ...
    init(selectLines, burn)
    {
        ...
        this.changeSelectedLinesEvent.add((l, b) => {if (this.infoText !== null) this.infoText.text = (l > 0) ? slotConfig.infoTextClickSpin : slotConfig.infoTextSelectLine;}, this);
        if(selectLines ==='all')
        {
    ...
    refresh()
    {
        ...
        this.refreshSpins();
        if (this.winAmountText != null) this.winAmountText.text = slotConfig.winAmountPrefix + 0;
    }

    refreshBetLines()
    {
        if (this.lineBetAmountText!= null) this.lineBetAmountText.text = slotConfig.lineBetPrefix + this.lineBet;
    ...
    refreshSpins()
    {
        ...
        if (this.freeSpinCountText != null) this.freeSpinCountText.text = (this.freeSpins > 0) ? slotConfig.freeSpinsAmountPrefix + this.freeSpins : '';
    }
    ...
    {
        if (this.freeSpinText != null) this.freeSpinText.text = (this.freeSpins > 0) ? "FREE" : "";
        if (this.freeSpinCountText != null) this.freeSpinCountText.text = (newFreeSpinsCount > 0) ? slotConfig.freeSpinsAmountPrefix + newFreeSpinsCount : "";
    }
    ...
    {
        if (this.lineBetAmountText != null) this.lineBetAmountText.text = slotConfig.lineBetPrefix + newLineBet;
    }
    ...
    {
       if(this.winAmountText != null) this.winAmountText.text = slotConfig.winAmountPrefix + newCount;
    }
    ...
    setJackpotAmount(amount)
    {
        ...
        if (changed) 
        {
          ...
          //  localStorage.setItem('mk_shopping_jp_amount', this.jackpotAmount); // save https://www.dynetisgames.com/2018/10/28/how-save-load-player-progress-localstorage/
        }
    }
    ...
    loadJackpot()
    {
      ...
      //  amount = parseInt(localStorage.getItem('mk_shopping_jp_amount')) || this.jackpotDefaultAmount; // load https://www.dynetisgames.com/2018/10/28/how-save-load-player-progress-localstorage/
